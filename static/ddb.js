document.addEventListener("DOMContentLoaded", () => {
  ...
  const disclaimer =
    "Ja sam Baltazar, tvoj virtualni asistent za programiranje i informatiku.";
  ...
  window.addEventListener("message", (event) => {
    ...
    switch (message.command) {
      ...
      case "say":
        addMessage(
          { text: message.content.userMessage, fromDuck: true },
          (askGpt = false),
        );
        break;
      ...
      case "delta_update":
        ...
        if (ddbChatMessage === null) {
          ...
          addMessage(
            { id: message.id, text: "", fromDuck: true },
            (askGpt = false),
          );

          // Markdown render
          document.querySelector(`#id-${message.id}`).innerHTML =
            message.content;
        } else {
      ...
      case "persist_messages":
        localStorage.setItem(
          "gptMessagesHistory",
          JSON.stringify(message.gpt_messages_array),
        );
        break;
    }
  });
  ...
  function getTiredMessage(id) {
    ...
    const chosenMessageId = Math.floor(Math.random() * tiredMessages.length);
  ...
  function addMessage({ id = uuidv4(), text, fromDuck }, askGpt = true) {
    const message = `<div class="ddbChat ${fromDuck ? "ddbChat-Duck" : "ddbChat-User"}">
                <span class="ddbChatBorder ${fromDuck ? "ddbChatBorder-Duck" : "ddbChatBorder-User"}"></span>
                <span class="ddbAuthorName"><b>${fromDuck ? "Baltazar" : "Vi"}</b></span>
                <span id="id-${id}" class="ddbChatMessage">${fromDuck && askGpt ? "..." : fromDuck ? md.render(text) : `<pre style="background-color: transparent; margin: 0; padding: 0; font-family: inherit; font-size: inherit; white-space: pre-wrap;">${text}</pre>`}</span>
            </div>`;
  ...
  function restoreMessages() {
    try {
      ...
      // restore messages
      let gptMessagesHistory = JSON.parse(
        localStorage.getItem("gptMessagesHistory"),
      );
          ...
          addMessage(
            {
              id: msg.id || uuidv4(),
              text: msg.content,
              fromDuck: msg.role === "assistant" ? true : false,
            },
            (askGpt = false),
          );
        });
  ...
  function getEnergy(regen = false) {
    try {
      ...
      if (regen) {
        ...
        ddbEnergy = Math.min(INITIAL_DDB_ENERGY, ddbEnergy + regenAmount);
      }
  ...
  textarea.addEventListener("keypress", (event) => {
    if (event.key === "Enter" && (event.ctrlKey || event.shiftKey)) {
      ...
      textBox.value =
        textBox.value.slice(0, textBox.selectionStart) +
        "\n" +
